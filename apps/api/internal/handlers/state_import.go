...
// Import kicks off a BACKGROUND CSV import of states. It streams the upload
...
func (h *StateHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsState(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "states", Status: "processing", Total: total}
	...
	go h.runImportState(job.ID, tmpPath)
...
// countCSVRowsState counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsState(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportState streams the temp CSV, creating states in batches and
...
func (h *StateHandler) runImportState(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.State, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "country"); ok && v != "" {
			var rel models.Country
			if err := h.DB.Where("name = ?", v).First(&rel).Error; err != nil {
				rel = models.Country{Name: v}
				h.DB.Create(&rel)
			}
			item.CountryID = rel.ID
		}
...
// Template returns a ready-to-fill CSV template (header row) for importing states.
...
func (h *StateHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="states-template.csv"`)
	c.String(http.StatusOK, "name,country\n")
}
