...
// Import kicks off a BACKGROUND CSV import of countries. It streams the upload
...
func (h *CountryHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsCountry(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
		c.JSON(http.StatusBadRequest, gin.H{
	...
	job := models.ImportJob{Resource: "countries", Status: "processing", Total: total}
	...
	go h.runImportCountry(job.ID, tmpPath)
...
// countCSVRowsCountry counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsCountry(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// runImportCountry streams the temp CSV, creating countries in batches and
...
func (h *CountryHandler) runImportCountry(jobID, tmpPath string) {
	...
	flush := func() {
		...
		items := make([]models.Country, len(batch))
		for i := range batch {
...
// Template returns a ready-to-fill CSV template (header row) for importing countries.
...
func (h *CountryHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="countries-template.csv"`)
