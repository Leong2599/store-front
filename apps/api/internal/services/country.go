package services

import (
	"fmt"
	"math"

	"gorm.io/gorm"

	"store-front/apps/api/internal/models"
)

// CountryService handles business logic for countries.
type CountryService struct {
	DB *gorm.DB
}

// CountryListParams holds pagination and filter parameters.
type CountryListParams struct {
	Page      int
	PageSize  int
	Search    string
	SortBy    string
	SortOrder string
}

// List returns a paginated list of countries.
func (s *CountryService) List(params CountryListParams) ([]models.Country, int64, int, error) {
	if params.Page < 1 {
		params.Page = 1
	}
	if params.PageSize < 1 || params.PageSize > 100 {
		params.PageSize = 20
	}
	if params.SortOrder != "asc" && params.SortOrder != "desc" {
		params.SortOrder = "desc"
	}
	// SortBy is interpolated into ORDER BY below, so it MUST be whitelisted
	// against real columns — never trust a client-supplied sort column.
	sortableCountry := map[string]bool{"id": true, "created_at": true, "updated_at": true, "name": true}
	if !sortableCountry[params.SortBy] {
		params.SortBy = "created_at"
	}

	query := s.DB.Model(&models.Country{})

	if params.Search != "" {
		query = query.Where("LOWER(name) LIKE LOWER(?)", "%"+params.Search+"%")
	}

	var total int64
	query.Count(&total)

	var items []models.Country
	offset := (params.Page - 1) * params.PageSize
	if err := query.Order(params.SortBy + " " + params.SortOrder).Offset(offset).Limit(params.PageSize).Find(&items).Error; err != nil {
		return nil, 0, 0, fmt.Errorf("fetching countries: %w", err)
	}

	pages := int(math.Ceil(float64(total) / float64(params.PageSize)))
	return items, total, pages, nil
}

// GetByID returns a single country by ID.
func (s *CountryService) GetByID(id string) (*models.Country, error) {
	var item models.Country
	if err := s.DB.First(&item, "id = ?", id).Error; err != nil {
		return nil, fmt.Errorf("country not found: %w", err)
	}
	return &item, nil
}

// Create creates a new country.
func (s *CountryService) Create(item *models.Country) error {
	if err := s.DB.Create(item).Error; err != nil {
		return fmt.Errorf("creating country: %w", err)
	}
	return nil
}

// Update modifies an existing country. Two queries: First() loads
// the row + verifies existence; Updates() persists the diff. The
// loaded struct is mutated by Updates() so we can return it directly
// without a third refetch.
func (s *CountryService) Update(id string, updates map[string]interface{}) (*models.Country, error) {
	var item models.Country
	if err := s.DB.First(&item, "id = ?", id).Error; err != nil {
		return nil, fmt.Errorf("country not found: %w", err)
	}

	if err := s.DB.Model(&item).Updates(updates).Error; err != nil {
		return nil, fmt.Errorf("updating country: %w", err)
	}

	return &item, nil
}

// Delete soft-deletes a country. One query — we don't need to load
// the row first; GORM's Delete is atomic and rows-affected tells us
// whether it existed.
func (s *CountryService) Delete(id string) error {
	res := s.DB.Where("id = ?", id).Delete(&models.Country{})
	if res.Error != nil {
		return fmt.Errorf("deleting country: %w", res.Error)
	}
	if res.RowsAffected == 0 {
		return fmt.Errorf("country not found")
	}
	return nil
}
